...
export default function TransformersSearch() {
	...
	const handleSearch = async () => {
		...
		try {
			...
			setResults([
				{ answer: "Error during search. Please try again.", sources: [] },
			]);
		} finally {
			setIsLoading(false);
		}
	...
	return (
		<div className="relative border rounded-lg sm:w-fit w-[68%]">
			...
			{isLoading && (
				<div className="absolute top-full left-0 w-full bg-white border rounded-md shadow-md p-4">
					Loading...
				</div>
			)}
			{results.length > 0 && (
									...
									<ul>
										{result.sources.map(
											(source: string, sourceIndex: number) => (
												<li key={sourceIndex} className="text-sm text-gray-500">
													Source: {source}
												</li>
											)
										)}
