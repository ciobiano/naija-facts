...
const protectedRoutes = {
	...
	public: ["/", "/docs", "/blog", "/api/auth", "/auth/verify-email", "/auth/reset-password"],
};

// Rate limiting for sensitive routes
const rateLimitMap = new Map<string, { count: number; resetTime: number }>();

function isRateLimited(ip: string, route: string, maxRequests: number = 10, windowMs: number = 60000): boolean {
	const key = `${ip}:${route}`;
	const now = Date.now();
...
export default withAuth(
	function middleware(req: NextRequest & { nextauth: { token: any } }) {
		...
		if (pathname.startsWith("/api/auth") || pathname.startsWith("/auth")) {
			if (isRateLimited(ip, pathname, 5, 60000)) { // 5 requests per minute
